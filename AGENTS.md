...
- Raw Git sync/integration/restore/Worktree cleanup and raw global installers remain blocked. Use the tested guarded fast-forward sync/integrate, exact unstage, integrated Worktree cleanup, and fixed-version winget installer so the beginner never has to run those commands.
