...
创建正式版本前，把逐维结论覆盖写入 `docs/RELEASE-REVIEW.md`（一版一份，覆盖更新；历史终审随版本标签保留在 Git 中）。首次创建该文件时把它加入 `docs/INDEX.md` 导航。guarded `version` 会拒绝：文件缺失、记录的版本与标签不一致、出现多条 `- Version:` 行、21 个维度缺行或重复行、固定维度键被改写、状态非法或证据/原因为空。补建历史标签时，历史树里已有的终审是权威：它记录了不同版本会直接拒绝，只有历史树完全没有终审文件时才接受当前检查点里该版本的终审。
...
终审门是静态门：只校验结构完整、版本匹配和证据非空，不能证明证据本身真实——伪造一行“测试通过”能骗过门禁，但骗不过审计。`audit-codex-kit` 按“结论强度不得高于证据”抽查 `verified` 证据是否可复现；伪造按 P1 处理。这是静态门的固有上限，不是缺陷；需要更高保证时，由用户或独立只读审查者复跑证据命令。
