...
2. 在 `docs/VERSIONS.md` 增加一行，描述用户能识别的能力、保留行为和验证；第一个版本建立时删除“No formal version”占位句。不要写 commit hash 或开发流水账。
3. 同步项目已有版本字段，例如 `package.json` 或根目录 `VERSION`。没有版本字段时不新增无意义清单；存在的字段必须与正式标签一致。
4. 把代码、测试、功能地图、STATUS、VERSIONS 和版本字段放进同一个最终检查点，避免随后再建“只记录完成”的第二个提交。
5. 检查点形成且工作树干净后运行：
...
该命令只允许标记当前已验证的 Dev Kit 检查点；标签已存在、版本索引缺失、项目版本字段不一致或工作树不干净时必须停止。禁止通过原始 Git 命令创建、移动、覆盖或删除旧版本标签；只读标签查询可以用于审计。
...
`--target` 只供 Codex 内部迁移使用，不要求用户提供提交编号。它拒绝非当前分支祖先、非 Dev Kit 检查点、版本字段不匹配和已存在的标签，不能成为任意移动标签的后门。
