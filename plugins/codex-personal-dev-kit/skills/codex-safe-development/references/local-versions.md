...
`--target` 只供 Codex 内部迁移使用，不要求用户提供提交编号。它拒绝非当前分支祖先、非 Dev Kit 检查点、版本字段不匹配和已存在的标签，不能成为任意移动标签的后门。历史检查点树中通常没有终审文件且不可改写；补建前先在当前检查点为该历史版本完成 21 维终审（`- Version:` 写历史版本号），门禁只在历史树完全没有终审文件时才接受当前 HEAD 中记录该版本的终审。历史树里已有终审但版本不符时会直接拒绝，不会被当前检查点掩盖——这防止把标签打到选错的历史提交上。
