...
- 项目存在 npm/pip/cargo 等生态时，顺带运行其自带的依赖审计命令（如 `npm audit`、`pip-audit`）并报告高危项；没有对应工具时如实标注未检查。
...
- 未经授权没有远程变更；明确授权的备份只通过 guarded publisher 推送精确 branch/tag，且 raw/force push、删除远程 refs、Release 和部署仍受阻止。
- 快进同步/线性整合、精确撤销暂存、已整合 Worktree 清理和固定版本 winget 首次安装是否只通过对应 guarded path；分叉历史、唯一文件/提交、隐式升级/降级必须被拒绝。
- 权限规则是否仍把可恢复、可精确验证的工作永久甩给零基础用户；需要新能力时应先建立独立受控通道，而不是直接放开原始命令。
