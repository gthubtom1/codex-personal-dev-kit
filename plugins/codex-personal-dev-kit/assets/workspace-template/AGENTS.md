...
| 发现自己在猜、没思路、方案摇摆；实现新能力/新集成前的默认调研；联网、GitHub、同类方案、现成源码或陌生/时效技术 | `$research-and-reuse` | 先分清不确定的类型再动手；受控只读研究、许可证/安全/兼容性评估和复用决策 |
...
- 它负责 UI/UX 判断和前端呈现；Git、后端架构、数据库、项目回滚或子代理相关流程由主流程和其他 Skill 承担。
...
- 卡住时先分清是哪一种不确定，三种走三条不同的路，只有第一种靠搜索：不确定**外面的世界是怎样的**（库的当前 API、报错别人怎么解、业界标准做法、版本/许可证/安全通告）→ 查官方文档和成熟实现；不确定**我们自己这套东西现在什么状态**（某函数实际行为、测试到底过没过、文件现在长什么样）→ 去跑去读源码和 Git，搜索在这里没用且会得出自信的错误结论；不确定**用户到底想要什么**（范围、取舍、可接受代价）→ 问用户，不替他假设。
- 状态本身就是触发条件：一旦发现自己正要凭印象给出关键结论、没有思路、或在两个方案之间摇摆，先查再动手。模型对自己的知识边界感知很差，会用与说对时完全相同的语气说错的事，“我记得”不是证据。
- 关键结论不能只靠自评。自己写、自己测、自己判“没问题”是最容易漏的一类；安全门、发布门和重要架构结论必须有第三方只读审查或外部证据支撑。什么才算证据见第 11 节。
...
调用有副作用的工具后，**验状态，不看回话**。回话是它说的，状态是它做的，两者经常不一致：清除截止时间的接口回“已重设”而截止根本没清；空契约下的 `close`／`cancel` 返回 exit 0 并打印“已封存”而实际什么也没发生；`git add .` 永远成功，所以它永远不会告诉你它刚卷进了别人的在途改动。凡是“总会成功”的写法都不报警，改可能有人同时在改的文件时宁可用会失败的精确字串替换。
...
- 原始/force Git、分叉历史自动 merge/rebase、删除或移动远程 refs、历史重写、强制清理或丢弃未确认工作仍禁止。发布、部署、生产迁移、包发布类任务由路由分派的产品流程处理；正式里程碑仅允许 guard-managed 本地语义标签，禁止原始 `git tag`。
...
- 一个 checkout 只有一个写入者；审查 subagent 默认只读。并行写入必须使用隔离 Worktree 和明确整合顺序。并行 Worktree 一律建到工作区外（用 guarded `worktree-path`，落在项目旁的 `../.<项目名>-worktrees/`），禁止建在工作区内部（如 `.local/`）——在有检查点快照/文件监视的宿主（Cursor / VSCode）里，工作区内的 Worktree 会被反复扫描与重打包，堆积后拖垮甚至冻结编辑器。这类宿主还需把 `__pycache__`、`*.sqlite3*`、`*.db` 等自动重生的产物加入该宿主的 `files.watcherExclude`/`search.exclude`（Cursor / VSCode 的 settings.json 或等价位置）。
...
永不原始、破坏性执行：raw/force Git、分叉历史自动整合、历史重写、远程 ref 删除、强制清理、删除未确认内容。发布、部署、生产变更、基础设施 apply/destroy 类任务由路由分派的产品流程处理。明确授权的备份/同步、线性整合、撤销暂存、Worktree 清理和工具安装走对应 guarded path。
...
- 接受的用户行为可用；
- 受影响和关键旧功能已复核；
- 相关测试、检查、构建或行为验证有真实命令证据（什么才算证据见下）；
...
结论强度不得高于证据等级。按你声称的是哪一种，分别适用下面四条：

- 说“这条保护存在”，要能说出把它改坏后哪一条会红。只有“全部通过”不算——守卫被整条删掉而套件一声不吭是常态，不是意外；全绿只说明量对了自己出的那张卷子。
- 说“已实现／已修”，依据必须是**执行它的代码**。schema、类型定义、配置、CI 里的一行、README 承诺、别人的审查报告都是**声明物**，可能没有任何代码在执行它；引用得再逐字正确也不算证据。
- 说“没有／零处／不存在”，先用不受忽略规则影响的方式复搜一遍（如 `rg --no-ignore`），中英文关键词各跑一遍；枚举类结论要从该概念的唯一写入口反推调用点，不靠单一语法特征数个数。
- 只在一个环境／一个版本／一台机器上跑过的，结论范围就收窄到那一格，其余显式写“未验证”。

修缺陷还有一条硬条件：**先红后绿，两份原始输出都要交。** 只交绿的那一次不算，因为无法分辨它是真修好了，还是又写了一条恒绿的假守卫。
