...
- The explicit model catalog is evidence for models that can be overridden directly; it is not a complete list of the current task's inherited model. If the current task/runtime confirms the parent is Luna and the spawn omits `model`, record the requested route provenance as `inherited-current-model` even when Luna is absent from that catalog. Only an accepted, started native spawn may set the effective model to Luna; a runtime rejection or missing result keeps the effective model `unconfirmed` and the status `failed`/`runtime-unconfirmed`.
...
- For an explicit roster such as “2 Sol and 3 Luna”, pass the exact model on each native spawn with `max` and judge the assignment from native argument acceptance/result. If the native runtime rejects or fails to confirm that explicit assignment, report that exact failure, keep the effective model `unconfirmed`, and mark the agent failed; do not silently substitute a model or reduce the requested count. A catalog omission alone is not enough to reject an inherited-current-model route.
