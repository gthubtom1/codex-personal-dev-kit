...
每个 `projects/<name>/` 是独立 Git 仓库，也是该项目的 Codex 主工作文件夹。母文件夹不建立总 Git 仓库，也不让一个任务同时修改多个项目。
...
- 一个 Codex 任务只负责一个连贯成果；新任务从短状态、功能地图、架构、测试和 Git 恢复。
- 项目文档保存当前事实并覆盖更新，不保存聊天全文、原始日志或无限增长的开发流水账。
...
- 工作区与项目模板、专业只读子代理、安装/诊断/审计/验证脚本
