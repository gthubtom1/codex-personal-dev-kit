...
- 74 unit tests cover feature contracts, meaningful STATUS freshness, all-active regression coverage, native edit/write guard behavior, exact checkpoints, reversible rollback, dirty-user-change isolation, bounded context recovery, document-bloat audits, dangerous command protection, standalone install/update/diagnosis, exact Skill path resolution, legacy-state detection, project creation, secret-aware first baselines, template-placeholder gates, domain/ADR document budgets, native-subagent boundaries, task-local model routing, config-default provenance, optional agent-list degradation, explicit legacy Luna migration, bounded structural validation, and standalone validator layout detection.
