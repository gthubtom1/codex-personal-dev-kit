...
- Mother folder: `D:\开发\AGENTS.md` contains the detailed system; workspace and project templates keep native multi-agent enabled, default Luna/max subagents, six concurrent slots, and no main-model lock. Codex resolves children as explicit per-spawn model/effort > project `[agents]` defaults > the current parent task model/effort; the ledger records requested/effective model and provenance instead of treating a selectable-model catalog as a complete inherited-model list.
...
- 70 unit tests cover feature contracts, meaningful STATUS freshness, all-active regression coverage, native edit/write guard behavior, exact checkpoints, reversible rollback, dirty-user-change isolation, bounded context recovery, document-bloat audits, dangerous command protection, standalone install/update/diagnosis, exact Skill path resolution, legacy-state detection, project creation, secret-aware first baselines, template-placeholder gates, domain/ADR document budgets, native-subagent boundaries, model-inheritance wording, bounded structural validation, and standalone validator layout detection.
...
- Native subagent policy requests Luna/max by default, supports multiple parallel agents and explicit Sol/Luna rosters at max; the roster ledger now distinguishes requested/effective model and provenance (`explicit-spawn`, `config-default`, `inherited-current-model`, `runtime-unconfirmed`). The system no longer rejects an inherited Luna solely because an explicit model catalog omits Luna; only native acceptance/start confirms the effective model, while Unknown model/rejection remains failed and unconfirmed. On August 2, 2026, the current parent task context was recorded as `gpt-5.6-luna`/`max`; an unpinned spawn routed through the configured Luna default and was rejected by the native runtime as unknown (the runtime listed Sol/Terra), while an explicit read-only Sol/max smoke test started successfully. Disabled `[agents].enabled` or `[features].multi_agent` gates are diagnosed and reported without being overridden.
