...
| `docs/RELEASE-REVIEW.md` | 当前正式版本的 21 维发布终审结论，一版一份覆盖更新；历史随版本标签保留在 Git 中 | 小于 120 行 |
...
- 普通检查点不等于产品版本。用户接受的完整里程碑先完成 21 维发布终审并覆盖写入 `docs/RELEASE-REVIEW.md`，再更新 `docs/VERSIONS.md`，在同一个最终检查点中保存代码、测试和文档，最后由 guarded `version` 创建本地 `vX.Y.Z` 标签；终审缺失、版本不匹配、维度缺行、状态非法或证据为空时 guard 拒绝打标签，如实标注的 `not-verified` 可以通过。
