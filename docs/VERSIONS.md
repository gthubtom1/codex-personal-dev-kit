...
| 版本 | 用户可识别的能力 | 验证与恢复状态 |
| --- | --- | --- |
| v0.2.11 | v0.2.10 的 10 智能体审查跟进修复：堵上一个 HIGH 分类器漏洞（`_catastrophic_delete` 现在在换行归一化之后跑、`$()` 提取改成配对括号，`echo x\nrm -rf /` / `format c:` / 嵌套 `$(...)` 不再溜过）；化解「不装 Hook」文档与已发 `.cursor/hooks.json` 的矛盾（限定为「不装会改 Codex 原生行为的 hook；发的是 fail-open 的 Cursor shell 守卫」）；把 watcher-exclude 那半补进两层 AGENTS + 项目模板；修过期文档（DESIGN 的 Handoff、README 映射表的 validate-kit.ps1/`.cursor`+`.vscode`/`$skill` 三处、self-check 限定 Cursor）；把新守卫测试接进 FEATURES DK-021；加强守卫测试 | guard `verify` 实跑 `Ran 223 tests OK`（221 + 2 新守卫：换行删除、嵌套 $()，exit 0，绑定全 22 个 DK）；`_catastrophic_delete`/嵌套 `$()` 修复以 11 组正反用例实测；`validate_kit` 结构校验通过后创建本地不可移动标签 v0.2.11；远程发布经用户授权走 guarded publish。已知延后到 v0.3：把 hook+watcher-exclude 迁移进已有项目、并重定位其工作区内 worktree |
